...
async function importWork(event){event.preventDefault();clearError();text(byId('import-feedback'),'Verificando integridad y copiando el bundle recibido…');const password=byId('import-password');try{if(!workspace())throw new Error('Indique un workspace local.');const bundle=byId('import-bundle').value.trim(),mode=byId('import-mode').value;if(!bundle||!byId('import-authorization').checked)throw new Error('Seleccione el bundle y confirme la autorización vigente.');if(mode==='ENCRYPTED'&&!password.value)throw new Error('Indique la password del bundle Encrypted.');const imported=mode==='ENCRYPTED'?await invoke('import_encrypted_work',{workspace:workspace(),bundle,password:password.value,authorizationRevalidated:true}):await invoke('import_plain_work',{workspace:workspace(),bundle,authorizationRevalidated:true});await openWorkspace();await loadSummary(imported.engagement.id,'Bundle verificado e importado. Original y provenance conservados. Revise autorización y Scope antes de reanudar.');text(byId('import-feedback'),`Trabajo recibido ${imported.engagement.name}; permanece pausado hasta revalidación.`);event.target.reset();byId('import-password-row').hidden=true}catch(error){text(byId('import-feedback'),'Importación rechazada; no se abrió un trabajo nuevo.');showError(error)}finally{password.value=''}}
...
byId('select-import-bundle').addEventListener('click',async()=>{try{const path=await invoke(byId('import-mode').value==='ENCRYPTED'?'select_encrypted_bundle':'select_plain_bundle');if(path)byId('import-bundle').value=path}catch(error){showError(error)}});
...
byId('import-form').addEventListener('submit',importWork);byId('import-mode').addEventListener('change',()=>{byId('import-bundle').value='';byId('import-password').value='';byId('import-password-row').hidden=byId('import-mode').value!=='ENCRYPTED'});
