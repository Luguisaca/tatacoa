...
El primer build release Linux detectó un requisito real de Tauri (`icons/icon.png`) ausente en el repositorio Windows. Se derivó `icon.png` del icono `.ico` ya existente, sin cambiar marca, y se incorporó a los paquetes Linux. El resultado posterior debe evaluarse desde el commit que contiene esta corrección; el intento fallido no cuenta como PASS.

El segundo intento compiló release en Linux, pero `dpkg-deb` rechazó los permisos `777` que WSL presenta para staging sobre `/mnt/c`. El script pasa a construir el staging bajo el filesystem nativo temporal de Linux y copia solo el resultado al workspace. AppImage sigue sin poder generarse en este host por ausencia local de `appimagetool`; no se sustituye por un formato diferente ni se atribuye PASS.

El tercer intento generó `.deb` y CLI `.tar.gz` con hashes; AppImage continuó NOT BUILT. Después se obtuvieron en el host WSL de build, fuera de los clientes QA, `appimagetool` 1.9.1 y el runtime x86_64 desde los [releases oficiales de appimagetool](https://github.com/AppImage/appimagetool/releases/tag/1.9.1) y [type2-runtime](https://github.com/AppImage/type2-runtime/releases/tag/continuous). Se verificaron los SHA-256 declarados por la API oficial: `ed4ce84f0d9caff66f50bcca6ff6f35aae54ce8135408b3fa33abfc3cb384eb0` y `1cc49bcf1e2ccd593c379adb17c9f85a36d619088296504de95b1d06215aebbf`, respectivamente. El script exige runtime local explícito para evitar descargas silenciosas durante el empaquetado. Los resultados AppImage posteriores se registran por separado.

Desde `7b47de9` el empaquetado Linux x64 final produjo tres candidatos locales en `target/dist/sp3-18e-linux-x64/`: Desktop `.deb` SHA-256 `4af60972d16a9fefad003f8769323c38731a40933f78357853c6fb4b255c37c9`, Desktop AppImage `05998d4f82afd502fcc6c4b9b53ad24fb26bcbefe944e0463ca0d71ab7c8ea47` y CLI `.tar.gz` `aa1246fa8553b250d2310b02c85f77ad80fb212d9ec25d2b6c73fbb2f55`. `sha256sum -c` de los tres, `dpkg-deb --info` y listado del tar: PASS. Instalación, primer arranque, flujo y cierre/reapertura en Kali/Parrot limpios: NOT TESTED, HUMAN QA pendiente. El AppImage usa WebKitGTK del sistema; el baseline de clientes constató su presencia, pero eso no constituye compatibilidad funcional.

## Bloque 19 — instalador Windows local

Rama `codex/sp3-19-windows-installer`, desde SP3-18. El [flujo oficial de Tauri 2 para Windows](https://v2.tauri.app/distribute/windows-installer/) ofrece NSIS/MSI. Se instaló `tauri-cli` 2.11.5 como herramienta de build bajo `target/tauri-tools`; `cargo-tauri build --bundles nsis --ci` produjo localmente un setup NSIS x64. La primera ejecución verificó y descargó las herramientas NSIS de Tauri con validación de hash; no hubo publicación. `scripts/package-windows-installer.ps1` conserva el commit de origen y SHA-256 del candidato. El setup no está firmado y requiere QA humano de instalación/primer arranque/cierre/reapertura/desinstalación en Windows 10 limpio.

Desde `3cd976f`, el instalador local `target/dist/sp3-19-windows-installer/tatacoa-0.1.0-alpha.1-3cd976f-windows-x64-desktop-setup.exe` obtuvo SHA-256 `ed30c1c5e144ace4b92c95308f3905340104222a98d010bb62253eb738bc7007`, recalculado con `Get-FileHash`. `Get-AuthenticodeSignature` devolvió `NotSigned`, consistente con la política de firma aún pendiente. No se instaló en el host de desarrollo como sustituto de QA humano en Windows 10 limpio.

## Bloque 20 — importación continuable Plain v2

Rama `codex/sp3-20-continuable-import`, desde SP3-19. Core implementa verify-before-trust y materialización de un bundle Plain v2 como Engagement continuable: copia original byte por byte bajo `received/plain`, IDs/provenance/contexto/historial conservados, objetos y relaciones comprobados otra vez desde staging, y continuidad inicial `PAUSED`. Colisiones, alteración, ausencia de autorización, claims de Evidence fuera de `CAPTURED`, perfiles HIGH_SENSITIVITY/CUSTOM Plain y versiones sin contexto completo se rechazan. App API, CLI y Desktop exponen la misma operación sin ejecutar recetas ni promover Evidence. El contrato y límites están en `specs/CONTINUABLE-IMPORT.md`.

QA técnico Windows: `cargo fmt`, `node --check`, Clippy estricto y `cargo test --locked --offline --workspace`: PASS. El test App API cubre importación positiva, copia retenida, historial/continuidad, rechazo sin revalidación, colisión, artifact alterado y claim de Evidence `VALIDATED`. Encrypted v1 recibido todavía no se materializa como trabajo; permanece pendiente para el siguiente bloque. QA humano de apertura/continuación de un proyecto recibido también permanece pendiente.

## Bloque 21 — importación continuable Encrypted v1

Rama `codex/sp3-21-encrypted-import`, desde SP3-20. Core autentica/verifica Encrypted v1 completo antes de staging, conserva una copia cifrada exacta, verifica esa copia y materializa objetos con autenticación y digest en staging invisible. Solo al finalizar todos los checks publica el Engagement en pausa con contexto/provenance/IDs históricos conservados. App API/CLI/Desktop exponen la operación; CLI solicita password sin eco, Desktop limpia el campo después de cada intento y advierte que el workspace materializado es plaintext. No hay promoción automática de Evidence ni ejecución de Replay.

QA técnico Windows: tests App API/Core, `cargo check --locked --offline --workspace --all-targets`, Clippy estricto y `node --check`: PASS. Se probaron password incorrecta, copia cifrada idéntica, lectura de artifact tras importar y rechazo de un byte alterado sin crear trabajo. La revisión integral de usuario y cliente limpio continúa pendiente; los artefactos distribuibles deben regenerarse desde el HEAD final.

## Bloque 22 — preparación de candidatos integrales

Rama `codex/sp3-22-alpha-qa-candidates`, desde SP3-21. El alcance técnico aprobado de Sprint 03 está implementado. Este bloque ejecuta QA acumulativo Windows/Linux y genera los formatos Desktop y CLI acordados desde un único commit limpio. Los SHA-256 y resultados efectivos se registran junto a los candidatos locales en `target/dist`; este registro no anticipa PASS de instalación o experiencia en clientes limpios. HUMAN-QA-05, el recorrido integral, la evaluación de Encrypted v1 dentro del producto y la decisión sobre firma/publicación siguen en gate humano.
